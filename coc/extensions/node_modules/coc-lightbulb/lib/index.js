...
var Config = class {
  ...
  setConfiguration() {
    const cfg = import_coc.workspace.getConfiguration("lightbulb");
    this.only = cfg.get("only", []);
    this.excludeFiletypes = cfg.get("excludeFiletypes", []);
    this.enableVirtualText = import_coc.workspace.isNvim && import_coc.workspace.nvim.hasFunction("nvim_buf_set_virtual_text") && cfg.get("enableVirtualText");
    const textCfg = import_coc.workspace.getConfiguration("lightbulb.text");
    this.text = {
      default: textCfg.get("default", "\uF834"),
      quickfix: textCfg.get("quickfix", "\uF567")
    };
    this.virtualTextPriority = cfg.get("virtualTextPriority");
    this.virtualTextPosition = cfg.get("virtualTextPosition");
    this.virtualTextPadding = 2 * Math.max(this.text.default.length, this.text.quickfix.length) + 2;
    this.enableSign = cfg.get("enableSign");
    this.signPriority = cfg.get("signPriority");
    this.followDiagnostic = cfg.get("followDiagnostic");
    const names = {
    ...
    if (this.enableSign) {
      ...
      nvim.command(`sign define ${names.sign.default} text=${this.text.default.replace(" ", "")} texthl=${names.sign.default} linehl=${names.sign.default}Line`, true);
      nvim.command(`sign define ${names.sign.quickfix} text=${this.text.quickfix.replace(" ", "")} texthl=${names.sign.quickfix} linehl=${names.sign.quickfix}Line`, true);
    }
    this.nvim6 = import_coc.workspace.has("nvim-0.6.0");
    if (this.nvim6) {
...
var Lightbulb = class {
  constructor() {
    this.nvim = import_coc2.workspace.nvim;
    this.nvim.createNamespace("coc-lightbulb").then((ns) => this.NS = ns);
  }
  async hasCodeActions(doc, only) {
    var _a;
    const none = {hasCodeActions: false, hasQuickFix: false};
    (_a = this.tokenSource) == null ? void 0 : _a.cancel();
    this.tokenSource = new import_coc2.CancellationTokenSource();
    const token = this.tokenSource.token;
    const range = await import_coc2.window.getSelectedRange("cursor");
    if (!range)
      return none;
    const diagnostics = import_coc2.diagnosticManager.getDiagnosticsInRange(doc.textDocument, range);
    const context = {diagnostics};
    if (only && only.length > 0) {
      context.only = only;
    }
    let codeActions = await import_coc2.languages.getCodeActions(doc.textDocument, range, context, token);
    ...
    if (codeActions.length) {
      return {
        hasCodeActions: true,
        hasQuickFix: codeActions.findIndex((action) => action.kind == import_vscode_languageserver_protocol.CodeActionKind.QuickFix) >= 0
      };
    }
    return none;
  }
  async show(doc, hasQuickFix) {
    const {buffer} = doc;
    buffer.setVar("coc_lightbulb_status", hasQuickFix ? config.text.quickfix : config.text.default);
    if (config.enableVirtualText)
      this.showVirtualText(doc, hasQuickFix);
    if (config.enableSign)
      buffer.placeSign({
        lnum: (await import_coc2.workspace.getCurrentState()).position.line + 1,
        name: hasQuickFix ? config.names.sign.quickfix : config.names.sign.default,
        group: "CocLightbulb",
        priority: config.signPriority
      });
  }
  async showVirtualText(doc, hasQuickFix) {
    const chunks = [
      hasQuickFix ? [config.text.quickfix, config.names.virtualText.quickfix] : [config.text.default, config.names.virtualText.default]
    ];
    const state = await import_coc2.workspace.getCurrentState();
    let lnum = state.position.line;
    if (lnum > state.document.lineCount - 1)
      return;
    if (!config.nvim6) {
    ...
    if (config.virtualTextPosition !== "auto") {
      buffer.setExtMark(this.NS, lnum, 0, {
        hl_mode: "combine",
        virt_text: chunks,
        virt_text_pos: config.virtualTextPosition,
        priority: config.virtualTextPriority
      });
      return;
    }
    const padding = config.virtualTextPadding;
    let col = 0;
  ...
  async clear(doc, force) {
    const {buffer} = doc;
    buffer.setVar("coc_lightbulb_status", "");
    if (force || config.enableVirtualText)
      buffer.clearNamespace(this.NS);
    if (force || config.enableSign)
      buffer.unplaceSign({group: "CocLightbulb"});
  }
  async refresh(forceClear) {
    const doc = await import_coc2.workspace.document;
    if (!doc || !doc.attached)
      return;
    if (config.excludeFiletypes.includes(doc.filetype))
      return;
    const buffer = doc.buffer;
    const disabled = await buffer.getVar("coc_lightbulb_disable") == 1 || config.followDiagnostic && await buffer.getVar("coc_diagnostic_disable") == 1;
    if (disabled) {
      await this.clear(doc, forceClear);
      return;
    }
    const {hasCodeActions, hasQuickFix} = await this.hasCodeActions(doc, config.only);
    await this.clear(doc, forceClear);
    if (hasCodeActions)
      await this.show(doc, hasQuickFix);
  }
};
...
async function activate(extCtx) {
  extCtx.subscriptions.push(import_coc3.workspace.onDidChangeConfiguration(async (e) => {
    if (e.affectsConfiguration("lightbulb")) {
      config.setConfiguration();
      await lightbulb.refresh(true);
    }
  }), import_coc3.events.on(["CursorHold", "CursorHoldI"], async () => {
