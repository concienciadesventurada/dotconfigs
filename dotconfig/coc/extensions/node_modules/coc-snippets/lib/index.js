    ...
    for (let snip of snippets) {
      ...
      if (ultisnip) {
        item.data.ultisnip = {
          ...
          range: import_coc5.Range.create(position.line, character, position.line, character + snip.prefix.length),
      ...
      if (snip.regex) {
        ...
        Object.assign(item, {
          textEdit: {
            range: import_coc5.Range.create({ line: position.line, character: character - head.length }, position),
            newText: snip.prefix
          }
        });
        ...
        Object.assign(item, {
          preselect: true,
          textEdit: {
            range: import_coc5.Range.create({ line: position.line, character: character - prefix.length }, position),
            newText: snip.prefix
          }
        });
      }
      ...
      if (!item.textEdit) {
        item.textEdit = {
          range: import_coc5.Range.create({ line: position.line, character }, position),
          newText: snip.prefix
        };
      }
      res.push(item);
...
var UltiSnippetsProvider = class extends BaseProvider {
  ...
  async showPrompt() {
    ...
    await import_coc8.window.showWarningMessage(`Ultisnips feature of coc-snippets requires python support on vim, check out :h ${name}`, {
...
async function activate(context) {
  ...
  async function doExpand(bufnr) {
    ...
    if (edits.length == 1) {
      ...
      let idx = await import_coc9.window.showQuickpick(edits.map((e) => e.description || e.prefix), "choose snippet:");
      if (idx == -1)
        return;
